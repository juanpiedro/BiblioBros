<?php
/*
 * Topchat_mentee.php – MySQL version
 */
...
try {
  $stmt = $pdo->prepare("SELECT c.active, u.fullname AS mentor_name FROM chats c JOIN requests r ON r.id = c.request_id JOIN users u ON u.id = r.mentor_id WHERE c.id = :cid LIMIT 1");
    ...
    <div class="row gx-4">
      ...
      <section class="col-md-8 d-flex flex-column" id="chat-window" style="display: none">
        <div class="chat-header mb-3 d-flex justify-content-between align-items-center">
          <h2 id="chat-with">Chat with <?= $mentorName ?></h2>
       </div>

          <?php if ($isActive): ?>
          <a href="rating.php?chat_id=<?= $chatId ?>" class="btn btn-warning btn-sm align-self-end mb-3">
  Close &amp; Rate Mentor
</a>
